...
function funcao_onclick(nome_elemento){
    ...
    if (confirm("Etiquetar como Fofinhos ? ")) {        
        httpGet(window.location.href+"registrando/"+nome_elemento);
        //console.log(nome_elemento);
//console.log(" httpGet  httpGet  httpGet  "+window.location.href+"registrando/"+nome_elemento);
        //console.log("SIM nome_elemento  "+window.location.href);
      } else {
...
//Desde a ultima IMAGEN (canto inferior direito) até a primeira (SUPERIOR ESQUERDO)
    ...
    indice_ultimo_slah=pre_url.split("/").length;
    nome_foto=pre_url.split("/")[indice_ultimo_slah-1];
...
//console.log("document.getElementById   "+nome_foto);
...
//console.log("url  "+url);
    ...
    if(document.readyState === "complete")
    {
        ...
        document.getElementById(nome_id).setAttribute('onclick', "funcao_onclick(\""+nome_foto+"\");");
    }
    else
    {
    //console.log("-----------------Falta---------------------");
